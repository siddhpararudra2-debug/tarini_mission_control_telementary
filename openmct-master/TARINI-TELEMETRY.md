...
To set up this side-by-side workflow—where Tarini Mission Control runs in one window and your custom Tarini dashboard runs locally in your web browser, updating live via a WebSocket bridge—follow these step-by-step instructions.
...
### 3. Start Your Tarini Local Server
**Terminal workflow.**  
Open your project directory in a terminal window where your custom code is located, and boot up the dashboard:
```bash
npm start
```
...
- **Local Dashboard**: [http://localhost:8080](http://localhost:8080) (Open your web browser and navigate to the address). You will see your custom Tarini mission control layout load up.

### 5. Arrange Windows Side-by-Side
**Visual setup.**  
Position your Tarini game window on one half of your monitor (or secondary monitor) and your browser window running Tarini dashboard right beside it. As you throttle up, pitch over, or stage in the game, the custom transport layer instantly streams the telemetry over WebSockets, updating your Tarini charts in real time.

### 6. Build a Custom Dashboard
**1. Open the Display Layout:** In your Tarini dashboard window, click on your "Unnamed Display Layout" item shown in the left tree menu or main viewport so that you are actively viewing/editing it.
