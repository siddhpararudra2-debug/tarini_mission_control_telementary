...
A real-time, comprehensive web-based mission control dashboard for Kerbal Space Program (KSP), built on top of [NASA's Open MCT (Open Mission Control Technologies)](https://nasa.github.io/openmct/).

This project integrates Kerbal Space Program with Open MCT via a custom WebSocket transport layer, allowing you to stream live flight telemetry, visualize orbital mechanics, and scroll back in time to review past mission data—just like a real space agency.
...
- **Developer Mock Server**: Includes a NodeJS mock telemetry server that simulates physically plausible rocket launches, allowing you to design and test dashboards without needing to boot up KSP.
...
- `/openmct-master` - The core Open MCT application.
  - `src/plugins/kspTelemetry` - The custom plugin logic that powers the KSP integration.
    - `dictionary.js` - Defines the telemetry mappings and data types.
    - `TelemachusTransport.js` - Handles the WebSocket connection, data validation, and historical buffering.
    - `KSPObjectProvider.js` - Defines the folder structure and pre-configured layouts.
    - `KSPTelemetryProvider.js` - Feeds data requests and subscriptions into Open MCT.
...
Follow these steps to run Kerbal Space Program in one window and your custom Open MCT instance locally in your web browser side-by-side.
...
Boot up Kerbal Space Program and launch a vessel onto the launchpad or into flight. Once the flight scene loads, Telemachus automatically starts hosting a local WebSocket data feed on port `8085`.
...
2. **Drag Parameters onto the Canvas:** Expand the *Kerbal Space Program > KSP Vessel* folder in the left sidebar tree. Click and drag the parameters you want to see directly onto the gray layout canvas.
